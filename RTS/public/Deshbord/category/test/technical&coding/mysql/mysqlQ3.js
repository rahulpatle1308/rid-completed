const questions = [
  {
    "num": 1,
    "question_en": "What does the MySQL command 'SHOW CREATE TABLE' do?",
    "question_hi": "MySQL कमांड 'SHOW CREATE TABLE' क्या करता है?",
    "options_en": [
      "SHOW CREATE TABLE",
      "OPTIMIZE TABLE",
      "DELETE",
      "DROP TABLE"
    ],
    "options_hi": [
      "SHOW CREATE TABLE",
      "OPTIMIZE TABLE",
      "DELETE",
      "DROP TABLE"
    ],
    "answer_en": "SHOW CREATE TABLE",
    "answer_hi": "SHOW CREATE TABLE",
    "attempted": false,
    "selected": ""
  },
  {
    "num": 2,
    "question_en": "What does the MySQL command 'DESCRIBE' do?",
    "question_hi": "MySQL कमांड 'DESCRIBE' क्या करता है?",
    "options_en": [
      "DESCRIBE",
      "SELECT",
      "OPTIMIZE TABLE",
      "ANALYZE"
    ],
    "options_hi": [
      "DESCRIBE",
      "SELECT",
      "OPTIMIZE TABLE",
      "ANALYZE"
    ],
    "answer_en": "DESCRIBE",
    "answer_hi": "DESCRIBE",
    "attempted": false,
    "selected": ""
  },
  {
    "num": 3,
    "question_en": "In MySQL, what does 'Replication' relate to?",
    "question_hi": "MySQL में 'Replication' किससे संबंधित है?",
    "options_en": [
      "Replication",
      "EXPLAIN",
      "SHOW PROCESSLIST",
      "OPTIMIZE TABLE"
    ],
    "options_hi": [
      "Replication",
      "EXPLAIN",
      "SHOW PROCESSLIST",
      "OPTIMIZE TABLE"
    ],
    "answer_en": "Replication",
    "answer_hi": "Replication",
    "attempted": false,
    "selected": ""
  },
  {
    "num": 4,
    "question_en": "In MySQL, what does 'Binary Log' relate to?",
    "question_hi": "MySQL में 'Binary Log' किससे संबंधित है?",
    "options_en": [
      "Binary Log",
      "SHOW VARIABLES",
      "SHOW STATUS",
      "SHOW PROCESSLIST"
    ],
    "options_hi": [
      "Binary Log",
      "SHOW VARIABLES",
      "SHOW STATUS",
      "SHOW PROCESSLIST"
    ],
    "answer_en": "Binary Log",
    "answer_hi": "Binary Log",
    "attempted": false,
    "selected": ""
  },
  {
    "num": 5,
    "question_en": "In MySQL, what does 'GTID' relate to?",
    "question_hi": "MySQL में 'GTID' किससे संबंधित है?",
    "options_en": [
      "GTID",
      "ANALYZE",
      "SHOW VARIABLES",
      "Primary-Replica"
    ],
    "options_hi": [
      "GTID",
      "ANALYZE",
      "SHOW VARIABLES",
      "Primary-Replica"
    ],
    "answer_en": "GTID",
    "answer_hi": "GTID",
    "attempted": false,
    "selected": ""
  },
  {
    "num": 6,
    "question_en": "In MySQL, what does 'Change Data Capture' relate to?",
    "question_hi": "MySQL में 'Change Data Capture' किससे संबंधित है?",
    "options_en": [
      "Change Data Capture",
      "ANALYZE",
      "FLUSH",
      "GTID"
    ],
    "options_hi": [
      "Change Data Capture",
      "ANALYZE",
      "FLUSH",
      "GTID"
    ],
    "answer_en": "Change Data Capture",
    "answer_hi": "Change Data Capture",
    "attempted": false,
    "selected": ""
  },
  {
    "num": 7,
    "question_en": "In MySQL, what does 'Master-Slave' relate to?",
    "question_hi": "MySQL में 'Master-Slave' किससे संबंधित है?",
    "options_en": [
      "Master-Slave",
      "Primary-Replica",
      "SHOW VARIABLES",
      "SHOW CREATE TABLE"
    ],
    "options_hi": [
      "Master-Slave",
      "Primary-Replica",
      "SHOW VARIABLES",
      "SHOW CREATE TABLE"
    ],
    "answer_en": "Master-Slave",
    "answer_hi": "Master-Slave",
    "attempted": false,
    "selected": ""
  },
  {
    "num": 8,
    "question_en": "In MySQL, what does 'Primary-Replica' relate to?",
    "question_hi": "MySQL में 'Primary-Replica' किससे संबंधित है?",
    "options_en": [
      "Primary-Replica",
      "GTID",
      "Replication",
      "SHOW VARIABLES"
    ],
    "options_hi": [
      "Primary-Replica",
      "GTID",
      "Replication",
      "SHOW VARIABLES"
    ],
    "answer_en": "Primary-Replica",
    "answer_hi": "Primary-Replica",
    "attempted": false,
    "selected": ""
  },
  {
    "num": 9,
    "question_en": "What is the purpose of the 'GRANT' command in MySQL?",
    "question_hi": "MySQL में 'GRANT' कमांड का उद्देश्य क्या है?",
    "options_en": [
      "GRANT",
      "SET PASSWORD",
      "CREATE TABLE",
      "DROP TABLE"
    ],
    "options_hi": [
      "GRANT",
      "SET PASSWORD",
      "CREATE TABLE",
      "DROP TABLE"
    ],
    "answer_en": "GRANT",
    "answer_hi": "GRANT",
    "attempted": false,
    "selected": ""
  },
  {
    "num":10,
    "question_en": "What is the purpose of the 'REVOKE' command in MySQL?",
    "question_hi": "MySQL में 'REVOKE' कमांड का उद्देश्य क्या है?",
    "options_en": [
      "REVOKE",
      "DELETE",
      "CREATE PROCEDURE",
      "FLUSH PRIVILEGES"
    ],
    "options_hi": [
      "REVOKE",
      "DELETE",
      "CREATE PROCEDURE",
      "FLUSH PRIVILEGES"
    ],
    "answer_en": "REVOKE",
    "answer_hi": "REVOKE",
    "attempted": false,
    "selected": ""
  },
  {
    "num":11,
    "question_en": "What is the purpose of the 'CREATE USER' command in MySQL?",
    "question_hi": "MySQL में 'CREATE USER' कमांड का उद्देश्य क्या है?",
    "options_en": [
      "CREATE USER",
      "DROP USER",
      "ALTER TABLE",
      "CREATE TABLE"
    ],
    "options_hi": [
      "CREATE USER",
      "DROP USER",
      "ALTER TABLE",
      "CREATE TABLE"
    ],
    "answer_en": "CREATE USER",
    "answer_hi": "CREATE USER",
    "attempted": false,
    "selected": ""
  },
  {
    "num":12,
    "question_en": "What is the purpose of the 'DROP USER' command in MySQL?",
    "question_hi": "MySQL में 'DROP USER' कमांड का उद्देश्य क्या है?",
    "options_en": [
      "DROP USER",
      "FLUSH PRIVILEGES",
      "DROP INDEX",
      "GRANT"
    ],
    "options_hi": [
      "DROP USER",
      "FLUSH PRIVILEGES",
      "DROP INDEX",
      "GRANT"
    ],
    "answer_en": "DROP USER",
    "answer_hi": "DROP USER",
    "attempted": false,
    "selected": ""
  },
  {
    "num":13,
    "question_en": "What is the purpose of the 'SET PASSWORD' command in MySQL?",
    "question_hi": "MySQL में 'SET PASSWORD' कमांड का उद्देश्य क्या है?",
    "options_en": [
      "SET PASSWORD",
      "FLUSH PRIVILEGES",
      "CREATE INDEX",
      "DROP TABLE"
    ],
    "options_hi": [
      "SET PASSWORD",
      "FLUSH PRIVILEGES",
      "CREATE INDEX",
      "DROP TABLE"
    ],
    "answer_en": "SET PASSWORD",
    "answer_hi": "SET PASSWORD",
    "attempted": false,
    "selected": ""
  },
  {
    "num":14,
    "question_en": "What is the purpose of the 'FLUSH PRIVILEGES' command in MySQL?",
    "question_hi": "MySQL में 'FLUSH PRIVILEGES' कमांड का उद्देश्य क्या है?",
    "options_en": [
      "FLUSH PRIVILEGES",
      "ALTER TABLE",
      "DELETE",
      "TRUNCATE"
    ],
    "options_hi": [
      "FLUSH PRIVILEGES",
      "ALTER TABLE",
      "DELETE",
      "TRUNCATE"
    ],
    "answer_en": "FLUSH PRIVILEGES",
    "answer_hi": "FLUSH PRIVILEGES",
    "attempted": false,
    "selected": ""
  },
  {
    "num":15,
    "question_en": "Which function returns the number of rows in a group?",
    "question_hi": "कौन-सा फ़ंक्शन किसी समूह में पंक्तियों की संख्या लौटाता है?",
    "options_en": [
      "COUNT()",
      "SUM()",
      "GROUP_CONCAT()",
      "AVG()"
    ],
    "options_hi": [
      "COUNT()",
      "SUM()",
      "GROUP_CONCAT()",
      "AVG()"
    ],
    "answer_en": "COUNT()",
    "answer_hi": "COUNT()",
    "attempted": false,
    "selected": ""
  },
  {
    "num":16,
    "question_en": "Which clause filters groups after aggregation?",
    "question_hi": "संग्रह के बाद समूहों को फ़िल्टर करने के लिए कौन-सा क्लॉज़ उपयोग होता है?",
    "options_en": [
      "HAVING",
      "WHERE",
      "GROUP BY",
      "ORDER BY"
    ],
    "options_hi": [
      "HAVING",
      "WHERE",
      "GROUP BY",
      "ORDER BY"
    ],
    "answer_en": "HAVING",
    "answer_hi": "HAVING",
    "attempted": false,
    "selected": ""
  },
  {
    "num":17,
    "question_en": "Which keyword eliminates duplicate rows from the result set?",
    "question_hi": "परिणाम सेट से duplicate rows हटाने के लिए कौन-सा keyword उपयोग होता है?",
    "options_en": [
      "DISTINCT",
      "UNION",
      "ALL",
      "GROUP BY"
    ],
    "options_hi": [
      "DISTINCT",
      "UNION",
      "ALL",
      "GROUP BY"
    ],
    "answer_en": "DISTINCT",
    "answer_hi": "DISTINCT",
    "attempted": false,
    "selected": ""
  },
  {
    "num":18,
    "question_en": "Which SQL construct provides a named temporary result set for a single query (CTE)?",
    "question_hi": "कौन-सा SQL construct एक नामित अस्थायी result set प्रदान करता है (CTE)?",
    "options_en": [
      "WITH",
      "CREATE VIEW",
      "TEMPORARY",
      "SUBQUERY"
    ],
    "options_hi": [
      "WITH",
      "CREATE VIEW",
      "TEMPORARY",
      "SUBQUERY"
    ],
    "answer_en": "WITH",
    "answer_hi": "WITH",
    "attempted": false,
    "selected": ""
  },
  {
    "num":19,
    "question_en": "What kind of function is ROW_NUMBER() in SQL?",
    "question_hi": "SQL में ROW_NUMBER() किस प्रकार का फ़ंक्शन है?",
    "options_en": [
      "ROW_NUMBER()",
      "COUNT()",
      "DATE_ADD()",
      "LAST_VALUE()"
    ],
    "options_hi": [
      "ROW_NUMBER()",
      "COUNT()",
      "DATE_ADD()",
      "LAST_VALUE()"
    ],
    "answer_en": "ROW_NUMBER()",
    "answer_hi": "ROW_NUMBER()",
    "attempted": false,
    "selected": ""
  },
  {
    "num":20,
    "question_en": "What kind of function is RANK() in SQL?",
    "question_hi": "SQL में RANK() किस प्रकार का फ़ंक्शन है?",
    "options_en": [
      "RANK()",
      "JSON_EXTRACT()",
      "COALESCE()",
      "ROW_NUMBER()"
    ],
    "options_hi": [
      "RANK()",
      "JSON_EXTRACT()",
      "COALESCE()",
      "ROW_NUMBER()"
    ],
    "answer_en": "RANK()",
    "answer_hi": "RANK()",
    "attempted": false,
    "selected": ""
  },
  {
    "num":21,
    "question_en": "What kind of function is DENSE_RANK() in SQL?",
    "question_hi": "SQL में DENSE_RANK() किस प्रकार का फ़ंक्शन है?",
    "options_en": [
      "DENSE_RANK()",
      "MAX()",
      "NOW()",
      "RANK()"
    ],
    "options_hi": [
      "DENSE_RANK()",
      "MAX()",
      "NOW()",
      "RANK()"
    ],
    "answer_en": "DENSE_RANK()",
    "answer_hi": "DENSE_RANK()",
    "attempted": false,
    "selected": ""
  },
  {
    "num":22,
    "question_en": "What kind of function is NTILE() in SQL?",
    "question_hi": "SQL में NTILE() किस प्रकार का फ़ंक्शन है?",
    "options_en": [
      "NTILE()",
      "MAX()",
      "UPPER()",
      "JSON_EXTRACT()"
    ],
    "options_hi": [
      "NTILE()",
      "MAX()",
      "UPPER()",
      "JSON_EXTRACT()"
    ],
    "answer_en": "NTILE()",
    "answer_hi": "NTILE()",
    "attempted": false,
    "selected": ""
  },
  {
    "num":23,
    "question_en": "What kind of function is LAG() in SQL?",
    "question_hi": "SQL में LAG() किस प्रकार का फ़ंक्शन है?",
    "options_en": [
      "LAG()",
      "SUBSTRING()",
      "JSON_OBJECT()",
      "DATE_SUB()"
    ],
    "options_hi": [
      "LAG()",
      "SUBSTRING()",
      "JSON_OBJECT()",
      "DATE_SUB()"
    ],
    "answer_en": "LAG()",
    "answer_hi": "LAG()",
    "attempted": false,
    "selected": ""
  },
  {
    "num":24,
    "question_en": "What kind of function is LEAD() in SQL?",
    "question_hi": "SQL में LEAD() किस प्रकार का फ़ंक्शन है?",
    "options_en": [
      "LEAD()",
      "UPPER()",
      "SUM()",
      "RANK()"
    ],
    "options_hi": [
      "LEAD()",
      "UPPER()",
      "SUM()",
      "RANK()"
    ],
    "answer_en": "LEAD()",
    "answer_hi": "LEAD()",
    "attempted": false,
    "selected": ""
  },
  {
    "num":25,
    "question_en": "What kind of function is FIRST_VALUE() in SQL?",
    "question_hi": "SQL में FIRST_VALUE() किस प्रकार का फ़ंक्शन है?",
    "options_en": [
      "FIRST_VALUE()",
      "SUM()",
      "DATE_SUB()",
      "JSON_OBJECT()"
    ],
    "options_hi": [
      "FIRST_VALUE()",
      "SUM()",
      "DATE_SUB()",
      "JSON_OBJECT()"
    ],
    "answer_en": "FIRST_VALUE()",
    "answer_hi": "FIRST_VALUE()",
    "attempted": false,
    "selected": ""
  },
  {
    "num":26,
    "question_en": "What kind of function is LAST_VALUE() in SQL?",
    "question_hi": "SQL में LAST_VALUE() किस प्रकार का फ़ंक्शन है?",
    "options_en": [
      "LAST_VALUE()",
      "UPPER()",
      "DATEDIFF()",
      "ROW_NUMBER()"
    ],
    "options_hi": [
      "LAST_VALUE()",
      "UPPER()",
      "DATEDIFF()",
      "ROW_NUMBER()"
    ],
    "answer_en": "LAST_VALUE()",
    "answer_hi": "LAST_VALUE()",
    "attempted": false,
    "selected": ""
  },
  {
    "num":27,
    "question_en": "Which function extracts a value from a JSON document by path?",
    "question_hi": "JSON दस्तावेज़ से path द्वारा मान निकालने के लिए कौन-सा फ़ंक्शन उपयोग होता है?",
    "options_en": [
      "JSON_EXTRACT()",
      "JSON_UNQUOTE()",
      "JSON_ARRAY()",
      "JSON_OBJECT()"
    ],
    "options_hi": [
      "JSON_EXTRACT()",
      "JSON_UNQUOTE()",
      "JSON_ARRAY()",
      "JSON_OBJECT()"
    ],
    "answer_en": "JSON_EXTRACT()",
    "answer_hi": "JSON_EXTRACT()",
    "attempted": false,
    "selected": ""
  },
  {
    "num":28,
    "question_en": "Which function converts a value to a JSON string?",
    "question_hi": "किस फ़ंक्शन से मान को JSON स्ट्रिंग में बदला जाता है?",
    "options_en": [
      "JSON_OBJECT()",
      "JSON_ARRAY()",
      "JSON_MERGE()",
      "TO_JSON()"
    ],
    "options_hi": [
      "JSON_OBJECT()",
      "JSON_ARRAY()",
      "JSON_MERGE()",
      "TO_JSON()"
    ],
    "answer_en": "JSON_OBJECT()",
    "answer_hi": "JSON_OBJECT()",
    "attempted": false,
    "selected": ""
  },
  {
    "num":29,
    "question_en": "Which tool is commonly used for logical backups of MySQL databases?",
    "question_hi": "MySQL डेटाबेस के logical backups के लिए कौन-सा टूल आमतौर पर उपयोग किया जाता है?",
    "options_en": [
      "mysqldump",
      "mysqlpump",
      "xtrabackup",
      "innobackupex"
    ],
    "options_hi": [
      "mysqldump",
      "mysqlpump",
      "xtrabackup",
      "innobackupex"
    ],
    "answer_en": "mysqldump",
    "answer_hi": "mysqldump",
    "attempted": false,
    "selected": ""
  },
  {
    "num":30,
    "question_en": "Which statement shows how MySQL executes a SELECT query (execution plan)?",
    "question_hi": "कौन-सा स्टेटमेंट दिखाता है कि MySQL SELECT query को कैसे execute करता है (execution plan)?",
    "options_en": [
      "EXPLAIN",
      "ANALYZE",
      "PROFILE",
      "SHOW TRACE"
    ],
    "options_hi": [
      "EXPLAIN",
      "ANALYZE",
      "PROFILE",
      "SHOW TRACE"
    ],
    "answer_en": "EXPLAIN",
    "answer_hi": "EXPLAIN",
    "attempted": false,
    "selected": ""
  },
  {
    "num":31,
    "question_en": "Which index type is typically used for range scans and ORDER BY optimization?",
    "question_hi": "range scans और ORDER BY optimization के लिए आमतौर पर किस index type का उपयोग किया जाता है?",
    "options_en": [
      "BTREE",
      "HASH",
      "FULLTEXT",
      "SPATIAL"
    ],
    "options_hi": [
      "BTREE",
      "HASH",
      "FULLTEXT",
      "SPATIAL"
    ],
    "answer_en": "BTREE",
    "answer_hi": "BTREE",
    "attempted": false,
    "selected": ""
  },
  {
    "num":32,
    "question_en": "Which SQL statement is used to perform the operation: SELECT?",
    "question_hi": "किस SQL कथन का उपयोग ऑपरेशन करने के लिए किया जाता है: SELECT?",
    "options_en": [
      "SELECT",
      "NOW()",
      "WHERE",
      "DELETE"
    ],
    "options_hi": [
      "SELECT",
      "NOW()",
      "WHERE",
      "DELETE"
    ],
    "answer_en": "SELECT",
    "answer_hi": "SELECT",
    "attempted": false,
    "selected": ""
  },
  {
    "num":33,
    "question_en": "Which SQL statement is used to perform the operation: INSERT?",
    "question_hi": "किस SQL कथन का उपयोग ऑपरेशन करने के लिए किया जाता है: INSERT?",
    "options_en": [
      "DATEDIFF()",
      "INSERT",
      "MIN()",
      "DATE"
    ],
    "options_hi": [
      "DATEDIFF()",
      "INSERT",
      "MIN()",
      "DATE"
    ],
    "answer_en": "INSERT",
    "answer_hi": "INSERT",
    "attempted": false,
    "selected": ""
  },
  {
    "num":34,
    "question_en": "Which SQL statement is used to perform the operation: UPDATE?",
    "question_hi": "किस SQL कथन का उपयोग ऑपरेशन करने के लिए किया जाता है: UPDATE?",
    "options_en": [
      "JOIN",
      "UNION",
      "COALESCE()",
      "UPDATE"
    ],
    "options_hi": [
      "JOIN",
      "UNION",
      "COALESCE()",
      "UPDATE"
    ],
    "answer_en": "UPDATE",
    "answer_hi": "UPDATE",
    "attempted": false,
    "selected": ""
  },
  {
    "num":35,
    "question_en": "Which SQL statement is used to perform the operation: DELETE?",
    "question_hi": "किस SQL कथन का उपयोग ऑपरेशन करने के लिए किया जाता है: DELETE?",
    "options_en": [
      "TIMESTAMP",
      "SUBSTRING()",
      "MIN()",
      "DELETE"
    ],
    "options_hi": [
      "TIMESTAMP",
      "SUBSTRING()",
      "MIN()",
      "DELETE"
    ],
    "answer_en": "DELETE",
    "answer_hi": "DELETE",
    "attempted": false,
    "selected": ""
  },
  {
    "num":36,
    "question_en": "Which SQL statement is used to perform the operation: TRUNCATE?",
    "question_hi": "किस SQL कथन का उपयोग ऑपरेशन करने के लिए किया जाता है: TRUNCATE?",
    "options_en": [
      "TRUNCATE",
      "LIMIT",
      "MAX()",
      "GROUP BY"
    ],
    "options_hi": [
      "TRUNCATE",
      "LIMIT",
      "MAX()",
      "GROUP BY"
    ],
    "answer_en": "TRUNCATE",
    "answer_hi": "TRUNCATE",
    "attempted": false,
    "selected": ""
  },
  {
    "num":37,
    "question_en": "Which SQL statement is used to perform the operation: REPLACE?",
    "question_hi": "किस SQL कथन का उपयोग ऑपरेशन करने के लिए किया जाता है: REPLACE?",
    "options_en": [
      "REPLACE",
      "CURDATE()",
      "TIMESTAMP",
      "HAVING"
    ],
    "options_hi": [
      "REPLACE",
      "CURDATE()",
      "TIMESTAMP",
      "HAVING"
    ],
    "answer_en": "REPLACE",
    "answer_hi": "REPLACE",
    "attempted": false,
    "selected": ""
  },
  {
    "num":38,
    "question_en": "Which SQL statement is used to perform the operation: CREATE TABLE?",
    "question_hi": "किस SQL कथन का उपयोग ऑपरेशन करने के लिए किया जाता है: CREATE TABLE?",
    "options_en": [
      "WHERE",
      "CREATE TABLE",
      "SUBSTRING()",
      "GROUP BY"
    ],
    "options_hi": [
      "WHERE",
      "CREATE TABLE",
      "SUBSTRING()",
      "GROUP BY"
    ],
    "answer_en": "CREATE TABLE",
    "answer_hi": "CREATE TABLE",
    "attempted": false,
    "selected": ""
  },
  {
    "num":39,
    "question_en": "Which SQL statement is used to perform the operation: ALTER TABLE?",
    "question_hi": "किस SQL कथन का उपयोग ऑपरेशन करने के लिए किया जाता है: ALTER TABLE?",
    "options_en": [
      "DROP TABLE",
      "JOIN",
      "CREATE PROCEDURE",
      "ALTER TABLE"
    ],
    "options_hi": [
      "DROP TABLE",
      "JOIN",
      "CREATE PROCEDURE",
      "ALTER TABLE"
    ],
    "answer_en": "ALTER TABLE",
    "answer_hi": "ALTER TABLE",
    "attempted": false,
    "selected": ""
  },
  {
    "num":40,
    "question_en": "Which SQL statement is used to perform the operation: DROP TABLE?",
    "question_hi": "किस SQL कथन का उपयोग ऑपरेशन करने के लिए किया जाता है: DROP TABLE?",
    "options_en": [
      "DROP TABLE",
      "DELETE",
      "AVG()",
      "JOIN"
    ],
    "options_hi": [
      "DROP TABLE",
      "DELETE",
      "AVG()",
      "JOIN"
    ],
    "answer_en": "DROP TABLE",
    "answer_hi": "DROP TABLE",
    "attempted": false,
    "selected": ""
  },
  {
    "num":41,
    "question_en": "Which SQL statement is used to perform the operation: CREATE INDEX?",
    "question_hi": "किस SQL कथन का उपयोग ऑपरेशन करने के लिए किया जाता है: CREATE INDEX?",
    "options_en": [
      "USING",
      "NOW()",
      "WHERE",
      "CREATE INDEX"
    ],
    "options_hi": [
      "USING",
      "NOW()",
      "WHERE",
      "CREATE INDEX"
    ],
    "answer_en": "CREATE INDEX",
    "answer_hi": "CREATE INDEX",
    "attempted": false,
    "selected": ""
  },
  {
    "num":42,
    "question_en": "Which SQL statement is used to perform the operation: DROP INDEX?",
    "question_hi": "किस SQL कथन का उपयोग ऑपरेशन करने के लिए किया जाता है: DROP INDEX?",
    "options_en": [
      "JOIN",
      "BLOB",
      "IFNULL()",
      "DROP INDEX"
    ],
    "options_hi": [
      "JOIN",
      "BLOB",
      "IFNULL()",
      "DROP INDEX"
    ],
    "answer_en": "DROP INDEX",
    "answer_hi": "DROP INDEX",
    "attempted": false,
    "selected": ""
  },
  {
    "num":43,
    "question_en": "Which SQL statement is used to perform the operation: CREATE VIEW?",
    "question_hi": "किस SQL कथन का उपयोग ऑपरेशन करने के लिए किया जाता है: CREATE VIEW?",
    "options_en": [
      "REPLACE",
      "DATEDIFF()",
      "CREATE VIEW",
      "DISTINCT"
    ],
    "options_hi": [
      "REPLACE",
      "DATEDIFF()",
      "CREATE VIEW",
      "DISTINCT"
    ],
    "answer_en": "CREATE VIEW",
    "answer_hi": "CREATE VIEW",
    "attempted": false,
    "selected": ""
  },
  {
    "num":44,
    "question_en": "Which SQL statement is used to perform the operation: CREATE PROCEDURE?",
    "question_hi": "किस SQL कथन का उपयोग ऑपरेशन करने के लिए किया जाता है: CREATE PROCEDURE?",
    "options_en": [
      "CREATE PROCEDURE",
      "CURDATE()",
      "JSON",
      "MIN()"
    ],
    "options_hi": [
      "CREATE PROCEDURE",
      "CURDATE()",
      "JSON",
      "MIN()"
    ],
    "answer_en": "CREATE PROCEDURE",
    "answer_hi": "CREATE PROCEDURE",
    "attempted": false,
    "selected": ""
  },
  {
    "num": 45,
    "question_en": "Which SQL statement is used to perform the operation: CREATE FUNCTION?",
    "question_hi": "किस SQL कथन का उपयोग ऑपरेशन करने के लिए किया जाता है: CREATE FUNCTION?",
    "options_en": [
      "CREATE FUNCTION",
      "REPLACE",
      "ORDER BY",
      "CREATE PROCEDURE"
    ],
    "options_hi": [
      "CREATE FUNCTION",
      "REPLACE",
      "ORDER BY",
      "CREATE PROCEDURE"
    ],
    "answer_en": "CREATE FUNCTION",
    "answer_hi": "CREATE FUNCTION",
    "attempted": false,
    "selected": ""
  },
  {
    "num": 46,
    "question_en": "Which clause is used for: WHERE?",
    "question_hi": "किस क्लॉज़ का उपयोग किया जाता है: WHERE?",
    "options_en": [
      "CURDATE()",
      "CREATE VIEW",
      "DELETE",
      "WHERE"
    ],
    "options_hi": [
      "CURDATE()",
      "CREATE VIEW",
      "DELETE",
      "WHERE"
    ],
    "answer_en": "WHERE",
    "answer_hi": "WHERE",
    "attempted": false,
    "selected": ""
  },
  {
    "num": 47,
    "question_en": "Which clause is used for: GROUP BY?",
    "question_hi": "किस क्लॉज़ का उपयोग किया जाता है: GROUP BY?",
    "options_en": [
      "SUM()",
      "GROUP BY",
      "CREATE PROCEDURE",
      "TRUNCATE"
    ],
    "options_hi": [
      "SUM()",
      "GROUP BY",
      "CREATE PROCEDURE",
      "TRUNCATE"
    ],
    "answer_en": "GROUP BY",
    "answer_hi": "GROUP BY",
    "attempted": false,
    "selected": ""
  },
  {
    "num": 48,
    "question_en": "Which clause is used for: ORDER BY?",
    "question_hi": "किस क्लॉज़ का उपयोग किया जाता है: ORDER BY?",
    "options_en": [
      "LEFT JOIN",
      "TRUNCATE",
      "DROP TABLE",
      "ORDER BY"
    ],
    "options_hi": [
      "LEFT JOIN",
      "TRUNCATE",
      "DROP TABLE",
      "ORDER BY"
    ],
    "answer_en": "ORDER BY",
    "answer_hi": "ORDER BY",
    "attempted": false,
    "selected": ""
  },
  {
    "num": 49,
    "question_en": "Which clause is used for: HAVING?",
    "question_hi": "किस क्लॉज़ का उपयोग किया जाता है: HAVING?",
    "options_en": [
      "COUNT()",
      "ORDER BY",
      "RIGHT JOIN",
      "HAVING"
    ],
    "options_hi": [
      "COUNT()",
      "ORDER BY",
      "RIGHT JOIN",
      "HAVING"
    ],
    "answer_en": "HAVING",
    "answer_hi": "HAVING",
    "attempted": false,
    "selected": ""
  },
  {
    "num": 50,
    "question_en": "Which clause is used for: LIMIT?",
    "question_hi": "किस क्लॉज़ का उपयोग किया जाता है: LIMIT?",
    "options_en": [
      "DISTINCT",
      "LIMIT",
      "SELECT",
      "FULL JOIN"
    ],
    "options_hi": [
      "DISTINCT",
      "LIMIT",
      "SELECT",
      "FULL JOIN"
    ],
    "answer_en": "LIMIT",
    "answer_hi": "LIMIT",
    "attempted": false,
    "selected": ""
  },
];


// --------------------------- GLOBAL VARS -----------------------------
let currentQuestion = 0;
let language = "en";
let timeLeft = 60 * 60; 
let timerInterval;

// ----------------- Load Question -----------------
function loadQuestion(index) {
    const q = questions[index];
    if (!q) return; //  undefined error fix

    document.getElementById("question").textContent =
        `${q.num}. ${language === "en" ? q.question_en : q.question_hi}`;

    document.getElementById("questionCounter").textContent =
        `Question ${index + 1} of ${questions.length}`;

    const optionsElement = document.getElementById("options");
    optionsElement.innerHTML = "";

    const options = language === "en" ? q.options_en : q.options_hi;

    options.forEach(option => {
        const isSelected = q.selected === option;

        const div = document.createElement("div");
        div.className = "option-box";

        div.style = `
            border: 2px solid ${isSelected ? "#007bff" : "#ccc"};
            background-color: ${isSelected ? "#e7f1ff" : "white"};
            padding: 10px;
            border-radius: 8px;
            margin: 6px 0;
            cursor: pointer;
        `;

        div.innerHTML = `
            <input type="radio" name="option" value="${option}" 
            ${isSelected ? "checked" : ""} style="margin-right:8px;">
            ${option}
        `;

        div.addEventListener("click", () => {
            markAttempted(index, option);
            loadQuestion(index);
        });

        optionsElement.appendChild(div);
    });

    updateNavigation();
}

// ----------------- Attempt Mark -----------------
function markAttempted(index, selectedAnswer) {
    questions[index].attempted = true;
    questions[index].selected = selectedAnswer;
    updateNavigation();
}

// ----------------- Next / Previous -----------------
function nextQuestion() {
    if (currentQuestion < questions.length - 1) {
        currentQuestion++;
        loadQuestion(currentQuestion);
    }
}

function prevQuestion() {
    if (currentQuestion > 0) {
        currentQuestion--;
        loadQuestion(currentQuestion);
    }
}

function changeLanguage() {
    language = document.getElementById("languageSelect").value;
    loadQuestion(currentQuestion);
}

// ----------------- Final Submit -----------------
function submitQuiz() {
    let confirmation = confirm("Are you sure you want to submit the test?");
    if (!confirmation) return;

    let attempted = 0;
    let notAttempted = 0;
    let score = 0;
    const results = [];

    questions.forEach(q => {
        if (q.attempted) {
            attempted++;

            if (q.selected === q.answer_en || q.selected === q.answer_hi) {
                score++;
            }
        } else {
            notAttempted++;
        }

        results.push({
            question: language === "en" ? q.question_en : q.question_hi,
            selected: q.selected || "Not Answered",
            correct: language === "en" ? q.answer_en : q.answer_hi
        });
    });

    localStorage.setItem("attempted", attempted);
    localStorage.setItem("notAttempted", notAttempted);
    localStorage.setItem("score", score);
    localStorage.setItem("results", JSON.stringify(results));

    let viewResult = confirm("Test submitted! Do you want to view result?");
    if (viewResult) {
        window.location.href = "/RTS/public/Deshbord/category/test/submit-test.html";
    }
}

// ----------------- Navigation Circles -----------------
function updateNavigation() {
    const nav = document.getElementById("circleContainer");
    nav.innerHTML = "";

    questions.forEach((q, i) => {
        let color = "gray";
        if (i === currentQuestion) color = "blue";
        else if (q.attempted) color = "green";

        nav.innerHTML += `
            <div class="circle" style="background:${color};"
            onclick="jumpToQuestion(${i})">${i + 1}</div>
        `;
    });
}

function jumpToQuestion(index) {
    currentQuestion = index;
    loadQuestion(index);
}

// ----------------- Timer -----------------
function startTimer() {
    const timerElement = document.getElementById("timer");

    timerInterval = setInterval(() => {
        if (timeLeft <= 0) {
            clearInterval(timerInterval);
            alert("Time's up!");
            submitQuiz();
        } else {
            const hours = String(Math.floor(timeLeft / 3600)).padStart(2, "0");
            const minutes = String(Math.floor((timeLeft % 3600) / 60)).padStart(2, "0");
            const seconds = String(timeLeft % 60).padStart(2, "0");

            timerElement.textContent = `Time Left: ${hours}:${minutes}:${seconds}`;
            timeLeft--;
        }
    }, 1000);
}

// ----------------- Camera & Movement Detection -----------------
let videoStream;
let movementCount = 0;

function startCamera() {
    const container = document.createElement("div");
    container.id = "camera-container";
    container.style = `
        position:fixed; top:10px; left:10px; width:130px; height:130px;
        border-radius:50%; overflow:hidden; border:3px solid red; z-index:9999;
    `;

    document.body.appendChild(container);

    const video = document.createElement("video");
    video.autoplay = true;
    video.playsinline = true;
    video.style = "width:100%; height:100%; object-fit:cover;";
    container.appendChild(video);

    navigator.mediaDevices.getUserMedia({ video: true })
        .then(stream => {
            video.srcObject = stream;
            videoStream = stream;
            detectMovement(video);
        })
        .catch(() => alert("Camera access denied!"));
}

function detectMovement(video) {
    const canvas = document.createElement("canvas");
    const ctx = canvas.getContext("2d");
    canvas.width = 160;
    canvas.height = 160;

    let lastData = null;

    setInterval(() => {
        ctx.drawImage(video, 0, 0, 160, 160);
        const data = ctx.getImageData(0, 0, 160, 160);

        if (lastData) {
            let diff = 0;
            for (let i = 0; i < data.data.length; i += 4) {
                diff += Math.abs(data.data[i] - lastData.data[i]);
            }

            if (diff > 1000000) {
                movementCount++;

                if (movementCount === 1) alert("⚠ Alert 1: No movement detected!");
                if (movementCount === 2) alert("⚠ Alert 2: Head not moving!");
                if (movementCount === 3) {
                    alert("⚠ Alert 3: Restarting test...");
                    restartTest();
                }
            }
        }
        lastData = data;

    }, 2000);
}

function restartTest() {
    if (videoStream) videoStream.getTracks().forEach(t => t.stop());

    const cam = document.getElementById("camera-container");
    if (cam) cam.remove();

    movementCount = 0;
    currentQuestion = 0;
    timeLeft = 60 * 60;

    questions.forEach(q => {
        q.attempted = false;
        q.selected = null;
    });

    loadQuestion(0);
    startTimer();
    startCamera();
}

// ----------------- Page Load -----------------
window.onload = function () {
    loadQuestion(currentQuestion);
    startTimer();
    startCamera();
};